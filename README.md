...
```bash
venv\Scripts\python main.py     # консольный интерфейс
venv\Scripts\python gui.py      # GUI (pywebview) — та же логика через Assistant
```
...
**GUI (`gui.py`):**
- Три экрана слева: Главный (голосовой орб + текстовый ввод-фолбэк),
  Команды (быстрый запуск приложений, системные действия с подтверждением,
  файловый браузер по белому списку), Настройки (голосовые профили, модель
  LLM, переключаются на лету без перезапуска).
- Клик по орбу — начать/остановить запись (замена Enter/Enter из консоли).
- Загрузка STT/TTS при старте занимает до минуты — показывается спиннер.

## Архитектура
`assistant.py` — единая точка правды для диалоговой логики (STT -> LLM
с tool calling -> TTS, переключение модели/голоса). `main.py` — тонкая CLI-
обёртка вокруг `Assistant`; тот же класс использует GUI (`gui.py`).
