...
```bash
venv\Scripts\python tui.py      # терминальный интерфейс (основной, рекомендуется)
venv\Scripts\python main.py     # простой консольный REPL (голос, без экранов)
...
Терминальное приложение на [Textual](https://textual.textualize.io/) — в духе
терминальных клиентов вроде самого Claude Code, без установщика и без
отдельного окна браузера. Три вкладки (переключаются мышью или Tab/Shift+Tab):
- **Главный** — журнал диалога + текстовый ввод (Enter — отправить). `R` —
  начать/остановить запись с микрофона (barge-in: новая запись обрывает то,
  что ассистент ещё договаривает).
- **Команды** — список приложений (Enter — открыть), системные действия
  (блокировка экрана / сон — через модалку подтверждения), файловый браузер
  по белому списку `config.yaml` (`files.root_dir`).
- **Настройки** — переключение голосового профиля и модели LLM на лету.
- `Q` — выход.

**GUI (`gui.py`, альтернатива):** то же самое в окне pywebview — три экрана
слева (Главный/Команды/Настройки), клик по орбу вместо клавиши `R`, слайдеры
NFE/CFG на экране настроек. Загрузка STT/TTS при старте занимает до минуты
(и там, и там) — показывается индикатор.

**Консоль (`main.py`, минимальный REPL без экранов):**
...
## Архитектура
`assistant.py` — единая точка правды для диалоговой логики (STT -> LLM
с tool calling -> TTS, переключение модели/голоса, barge-in). Три интерфейса
(`tui.py`, `gui.py`, `main.py`) — тонкие обёртки вокруг одного `Assistant`,
без дублирования логики.

## Инструменты (tool calling) и whitelist
LLM может вызывать только явно перечисленные инструменты — никаких
произвольных команд:
- `open_app` — открыть приложение из `config.yaml` (`apps` + русские алиасы
  `aliases`).
- `open_file` — открыть файл в приложении по умолчанию; путь ограничен
  директорией `config.yaml` (`files.root_dir`), выход за её пределы (`../..`,
  абсолютные пути) отклоняется на уровне `tools._resolve_within_root`.

Ещё есть `list_dir`/`read_file` (для файлового браузера в GUI) и
`lock_screen`/`sleep_system` (для панели «Система» в GUI) — они **не**
входят в `TOOL_SCHEMA` и голосом не вызываются: экран блокировки/сон — это
осознанно только явное действие в интерфейсе, не то, что стоит доверять
случайной голосовой команде.
...
## Задержка синтеза и barge-in
Измерено на стенде (AMD RX 9060 XT, ROCm) — `tts.infer()` для одной фразы,
после первого прогрева модели (первый вызов в процессе всегда медленнее —
разовая компиляция GPU-ядер, ~7с сверху, дальше не повторяется):

| `nfe_step` | латентность |
|---|---|
| 32 (дефолт) | ~1.7с |
| 16 | ~0.9с |
| 8  | ~0.45с |

Ниже `nfe_step` — быстрее, но менее разборчиво звучит голос; компромисс
субъективный, поэтому вынесен в слайдер на экране «Настройки» (GUI) —
можно подобрать на слух вместо правки конфига и перезапуска. Раньше в этом
README фигурировали цифры 7-10с — это был необновлённый замер, включавший
разовый прогрев, реальная установившаяся задержка заметно ниже.

**Узкое место — не TTS, а STT.** Распознавание (whisper `medium` int8 на
CPU) на том же стенде даёт 5.86с на 9.61с аудио (RTF 0.61x) — в 3.4 раза
дольше синтеза. Перевод STT на GPU — приоритет №1, подробности и варианты
для NVIDIA/AMD в [ROADMAP.md](ROADMAP.md).

Barge-in: начало новой записи (`R` в TUI / клик по орбу в GUI) сразу
обрывает то, что ассистент ещё договаривает (`TTSEngine.interrupt()`) —
не нужно ждать, пока он закончит фразу.

## Известные ограничения прототипа
- Push-to-talk (клавиша/клик), а не voice activity detection — ассистент
  не понимает, что вы начали говорить, без явного действия.
- Три интерфейса (TUI/GUI/консоль) сосуществуют осознанно — TUI и GUI
  дублируют экраны, но не логику (оба тонкие обёртки над `Assistant`);
  если это станет обузой, один из них можно будет убрать.
