...
-> TTS (Silero, потоково по предложениям)."""
...
SYSTEM_PROMPT = (
    "Ты — голосовой ассистент по имени Джарвис. Отвечай на русском языке, "
...
def main():
    print(f"Загружаю STT (faster-whisper)...")
    stt = STTEngine(model_size="medium")
    print("Загружаю TTS (Silero)...")
    tts = TTSEngine()
    speaker = StreamingSpeaker(tts)

    messages = [{"role": "system", "content": SYSTEM_PROMPT}]

    print("\n=== Джарвис готов. Ctrl+C для выхода. ===")
    try:
        while True:
